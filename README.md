...
- `luminosity/dumpRunLumiFromTree.C`  
  ROOT macro that scans the final JPsiTrkTrkTrk TTree in `muonia_all_jpsitrktrktrk_merged.root` and extracts all unique `(run, lumi)` pairs that contribute to the selected candidates. In the ROOT ntuple, the branches are named `run` and `lumi`, where `lumi` corresponds to the CMS lumisection index (LS). The macro writes these pairs to a plain-text file (by default `runls_muonia.txt`), one `run lumi` pair per line, sorted. In the rest of this directory and in the luminosity calculation, these entries are conceptually treated as `(run, ls)` pairs.
...
- `luminosity/muonia_lumi/runls_muonia.txt`  
  Plain-text list of all `(run, lumi)` pairs that contribute to the selected JPsiTrkTrkTrk candidates in `muonia_all_jpsitrktrktrk_merged.root`. Each line contains two integers, `run` and `lumi`, where `lumi` is the lumisection number (LS) as stored in the ROOT TTree. This file is produced by the ROOT macro `dumpRunLumiFromTree.C`, contains no duplicated pairs, and is already sorted. In subsequent steps (`runls_to_json.py`, `brilcalc`, and the CSV-based computation), these pairs are treated as `(run, ls)` input for the effective-luminosity workflow.
...
Further documentation on the luminosity inputs and methodology:

- The 2011 `lumibyls` CSV tables used here (`2011lumibyls_pxl.csv` and `2011lumibyls_hfoc.csv`) are taken from the CMS luminosity information for 2011 data on the CERN Open Data portal:  
  https://opendata.cern.ch/record/1053

- The overall procedure for computing integrated luminosity with `brilcalc`, JSON files, and calibration tags (data tags / normtags) follows the official CMS Open Data guide to luminosity calculation:  
