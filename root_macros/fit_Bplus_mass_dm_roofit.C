...
static void drawCmsHeader(TPad *pad,
                          const char *text = "CMS, #sqrt{s} = 7 TeV, L = 217.8 pb^{-1}")
{
  // Pad margins
  const double xmin = pad->GetLeftMargin();
  const double xmax = 1.0 - pad->GetRightMargin();
                    ...
                    double& sigma_val,double& sigma_err)
{
  ...
  drawCmsHeader(&c, "CMS, #sqrt{s} = 7 TeV, L = 217.8 pb^{-1}");
...
void fit_Bplus_mass_dm_roofit(const char* infile="muonia_all_jpsitrktrktrk_merged.root",
